...
def _(dlt, pd):
    ...
    def run_sql(sql: str) -> "pd.DataFrame":
        """Run SQL against the pipeline's playground dataset -> DataFrame."""
...
def _(mo, pipeline):
    ...
    Source: dlt pipeline **`agent_traces`** → dataset `{pipeline.dataset_name}`
    in the playground destination.
    """)
    return
